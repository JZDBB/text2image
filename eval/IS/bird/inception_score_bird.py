...
tf.app.flags.DEFINE_string('rfile', '', """Path for result """)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            with tf.device("/gpu:%d" % FLAGS.gpu):
                ...
                mean, var = get_inception_score(sess, images, pred_op)
                with open(FLAGS.rfile, "a+") as f:
                    f.write("score: {}, var: {}".format(mean, var))
                    f.close()
