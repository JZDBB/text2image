...
class Vgg16(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        self.vgg_pretrained_classifier = models.vgg16(pretrained=True)
	...
	#print X.shape
	#print self.vgg_pretrained_classifier,'-----',self.vgg_pretrained_features
        #X = self.vgg_pretrained_features(X)
        #X = X.view(X.size(0), -1)
        X = resize2d(X, (224,224))
        cate_label = self.vgg_pretrained_classifier(X)
        return out,cate_label
class Vgg16_ori(torch.nn.Module):
    def __init__(self, requires_grad=False):
