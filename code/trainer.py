...
class condGANTrainer(object):
    ...
    def build_models(self):
        ...
        if cfg.TRAIN.NET_G != '':
            ...
            if cfg.TRAIN.B_NET_D:
                ...
                Mname = '%s/netD_mask.pth' % (s_tmp)
                state_dict = \
                    torch.load(Mname, map_location=lambda storage, loc: storage)
                D_MASK.load_state_dict(state_dict)
        # ########################################################### #
        if cfg.CUDA:
            ...
            for i in range(len(netsD)):
                netsD[i].cuda()
            D_MASK.cuda()
        return [text_encoder, image_encoder, netG, netsD, D_MASK, epoch]

    def define_optimizers(self, netG, netsD, D_mask):
        optimizersD = []
        num_Ds = len(netsD)
        ...
        optimizerM = optim.Adam(D_mask.parameters(),
                                lr=cfg.TRAIN.DISCRIMINATOR_LR,
                                betas=(0.5, 0.999))
    ...
    def train(self):
        text_encoder, image_encoder, netG, netsD, D_mask, start_epoch = self.build_models()
        avg_param_G = copy_G_params(netG)
        optimizerG, optimizersD, optimizerM = self.define_optimizers(netG, netsD, D_mask)
