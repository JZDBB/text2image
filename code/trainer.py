...
import time
import numpy as np
from miscc.graph import param

# ################# Text to image task############################ #
class condGANTrainer(object):
    def __init__(self, output_dir, data_loader, n_words, ixtoword, dataset, args):
        if cfg.TRAIN.FLAG:
    ...
    def define_optimizers(self, netG, netsD):
        ...
        optimizerE = optim.Adam(netG.parameters(),
                                lr=cfg.TRAIN.GENERATOR_LR*0.2,
                                betas=(0.5, 0.999))

        return optimizerG, optimizersD, optimizerE
    ...
    def save_model(self, netG, avg_param_G, netsD, text_encoder, epoch):
        ...
        statu = {
            ...
            'optimizer': self.optimizerE.state_dict()
        }
    ...
    def train(self):
        ...
        self.optimizerG, self.optimizersD, self.optimizerE = self.define_optimizers(netG, netsD)
        real_labels, fake_labels, match_labels = self.prepare_labels()
        param(netG, "netG")
        # for i in xrange(len(netsD)):
        #     param(netsD[i], "netD{}".f)
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            if epoch % cfg.TRAIN.SNAPSHOT_INTERVAL == 0:  # and epoch != 0:
                self.save_model(netG, avg_param_G, netsD, text_encoder, epoch)

        self.save_model(netG, avg_param_G, netsD, text_encoder, self.max_epoch)
